import { z as attr_style, x as ensure_array_like, F as attr_class, G as stringify, y as bind_props, w as slot } from "../../../../chunks/index.js";
...
function StatsTable($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let i = 0, $$length = each_array_2.length; i < $$length; i++) {
      ...
      const each_array_3 = ensure_array_like(statCols);
      for (let $$index_2 = 0, $$length2 = each_array_3.length; $$index_2 < $$length2; $$index_2++) {
        let k = each_array_3[$$index_2];
        $$renderer2.push(`<div class="py-1 text-center">${escape_html(getStat(row, k))}</div>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    }
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (data.streamed?.payload) {
      data.streamed.payload.then((res) => {
